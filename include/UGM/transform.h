...
namespace Ubpa {
	...
	struct transform : SI<transform<F>> {
		...
		static const transform look_at(
			const point<F, 3>& pos,
		...
		static const transform orthographic(F width, F height, F zNear, F zFar) noexcept;
		// perspective, camera space -> clip space
		// fovy: verticle field of view in radian
		// aspect : width / height
		static const transform perspective(F fovy, F aspect, F zNear, F zFar,
			F near_clip_vlaue = UBPA_DEFAULT_NEAR_CLIP_VALUE) noexcept;

		// sample: rotate_with<Axis::X>(to_radian(theta))
		template<Axis axis>
		static const transform rotate_with(F theta) noexcept;

		const vec<F, 3> decompose_translation() const noexcept;
		const mat<F, 3> decompose_rotation_matrix() const noexcept;
		const quat<F> decompose_quatenion() const noexcept;
		const euler<F> decompose_euler() const noexcept;
		const mat<F, 3> decompose_mat3() const noexcept;
		const scale<F, 3> decompose_scale() const noexcept;
		...
		const hvec<F, 4> operator*(const hvec<F, 4>& hv) const noexcept;
		const point<F, 3> operator*(const point<F, 3>& p) const noexcept;
		const vec<F, 3> operator*(const vec<F, 3>& v) const noexcept;
		// result isn't normalized
		const normal<F> operator*(const normal<F>& n) const noexcept;
		const bbox<F, 3> operator*(const bbox<F, 3>& b) const noexcept;
		const line<F, 3> operator*(const line<F, 3>& r) const noexcept;
		const ray<F, 3> operator*(const ray<F, 3>& r) const noexcept;
	};
